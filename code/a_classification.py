#!/usr/bin/env python3
"""
June 4, 2025: I copied over this script from doublerainbow. It's the most up-to-date version.
/home/seguraab/ara-kinase-prediction/code/5a_classification.py

June 4, 2025: [BUG FIX] I added -drop_na flag; fixed the code block beginning at
line 610; added a try/except at line 195.

June 5, 2025: [BUG FIX] I added f1_macro for multiclass classification in AutoGluon.
...
Required Inputs
	...
	-drop_na Whether to drop rows with missing values (y/n, default is n)
...
from b_feature_selection import feature_selection_clf
...
def create_balanced(X, n, y_name):
    ...
    for b in range(num_datasets):
        # Downsample the majority classes to create a balanced dataset
        undersampler = RandomUnderSampler(
            random_state=b, sampling_strategy="auto", replacement=False)

        # Apply to your data
        X_resampled, y_resampled = undersampler.fit_resample(X, y)
        balanced_data = pd.concat(
            [y_resampled, X_resampled], axis=1, ignore_index=False)
        ...
        print("Downsampled class distribution:", Counter(y_resampled))
    ...
    if X.isnull().values.any():
        if args.drop_na == "y":
            ...
            print("Dropping rows with NAs... dim change: %s to %s."
                  % (str(start_dim), str(X.shape)))

    print(f"Label: {args.y_name}")
    if args.Y == "":  # get the label from X or Y files
        ...
        if y.isna().any():
            if args.drop_na == 'y':
                ...
                start_dim_X = len(X)
                y = y.dropna()
                print(f"Number of y rows went from {start_dim} to {len(y)}")
                print(f"Number of X rows went from {start_dim_X} to {len(X)}")
        X = X.loc[y.index, :]
